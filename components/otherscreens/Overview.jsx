...
const Overview = ({ userImageTab }) => {
    ...
    const [current, setCurrent] = useState({ data: userData.tabImg[0], index: 0 })
    console.log(userData.tabImg[0])
            ...
            <React.Fragment>
                <View style={styles.container}>
                    ...
                    <View style={styles.imageContainer}>
                        ...
                        <View style={styles.statusTabContainer}>
                            {
                                userData.tabImg.map((data, index) =>
                ...
                <TouchableOpacity onPress={() => {
                    if (current.index === 0) { console.log(current) }
                        ...
                        data: userData.tabImg[current.index - 1]
                    })
                }}
                ...
                <TouchableOpacity onPress={() => {
                    if (current.index === userData.tabImg.length - 1) { console.log(current) }
                    else setCurrent({
                        ...current,
                        index: current.index + 1,
                        data: userData.tabImg[current.index + 1]
        ...
        <View style={styles.container}>
            ...
            <View style={{ height: 100, width: WIDTH * 0.9, position: "absolute", bottom: 150, alignSelf: "center", flexDirection: "column", justifyContent: "space-between" }}>
                ...
                <View style={{ flexDirection: "row", }}>
                    <Text style={{ fontWeight: "bold", fontSize: 28, color: "#eee" }}>{userData.name},</Text>
                    <Text style={{ fontWeight: "500", fontSize: 28, color: "#eee", marginLeft: 5 }}> {userData.age}</Text>
                </View>


                <View style={{ flexDirection: "row", }}>
                    <FontAwesome5 name="house-user" size={12} color="#eee" />
                    <Text style={{ fontWeight: "bold", color: "#eee", textAlign: "center", fontSize: 10, marginLeft: 2 }}>Vit à  {userData.ville}</Text>
                </View>
                <View style={{ flexDirection: "row", }}>
                    <EvilIcons name="location" size={13} color="white" />
                    <Text style={{ fontWeight: "bold", color: "#eee", textAlign: "center", fontSize: 10, marginLeft: 2 }}> {userData.metier}</Text>
                </View>
            </View>
